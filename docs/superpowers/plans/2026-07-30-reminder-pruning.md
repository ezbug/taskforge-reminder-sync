...
- Core 提供共享的私有运行时目录初始化：普通写路径只迁移本人拥有、非
  symlink、无扩展 ACL、且 group/world 不可写的旧目录；目录和文件分别
  收紧为 `0700` / `0600`，并在 descriptor 上 `fstat` 复核。反向源备份与
  prune store 使用同一套策略。
- `--prune-dry-run` 不负责迁移：目录不存在时按空账本处理；既有目录不是
  严格 `0700` 时匿名失败，并且不创建文件、不 `chmod`。普通同步、watcher
  的 prune 推进或反向备份写入会在写操作前完成安全迁移。
...
- watcher 无限循环本轮不新增伪造的直接测试：协调顺序由
  ...
  测试覆盖。真实 watcher 生命周期仍保留在部署验收步骤 3–5，本轮禁止启动
  production EventKit 或 LaunchAgent。
